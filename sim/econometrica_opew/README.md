...
$$\text{Bias} \propto \|\hat{\gamma} - \gamma_0\| \times \|\hat{\nu} - \nu_0\| = O(T^{-\zeta}) \times O(T^{-\zeta}) = O(T^{-2\zeta}) \tag{13}$$
...
$$r(b; c) \equiv \mathbb{P}_{Z \sim \mathcal{N}(0,1)}(|Z + b| > c) = \Phi(-c - b) + \Phi(-c + b) \tag{25}$$
